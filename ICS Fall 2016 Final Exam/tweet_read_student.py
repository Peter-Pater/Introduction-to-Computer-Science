...
if __name__ == "__main__":
    ...
    for i in range(num_reads):
        ...
        update_interestingness = []
        # explore to select the "pick"
        if player_dice.roll() == True:
            pick = int(random.uniform(0,3))
        else:
            for i in range(num_twitters):
                update_interestingness.append(my_feeds[i]['likes']/my_feeds[i]['reads'])
            max_interest = max(update_interestingness)
            pick = update_interestingness.index(max_interest)
